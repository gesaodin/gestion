...
class Usuario extends CI_Model {
  ...
  function actualizar(){
    ...
    $sActualizar = 'UPDATE usuario SET clave=\'' . $this->_claveEncriptada() . '\' WHERE id = \'' . $this->id . '\'';
    //echo $sActualizar;
  ...
  public function existe() {
    $codigo = -1;
    $consulta = 'SELECT id FROM usuario WHERE login =\'' . $this -> cedula . ' \' LIMIT 1';
  ...
  function obtener($id){
    $sConsulta = 'SELECT * FROM usuario WHERE id=\'' . $id . '\'';
    $obj = $this->Dbpace->consultar($sConsulta);
    return $obj->rs;
  }

  function listar(){   
    $sConsulta = "SELECT id, login, nombre, apellido, status_id FROM usuario WHERE status_id IN (292, 293) ORDER BY id";
    $obj = $this->Dbpace->consultar($sConsulta);
    return $obj->rs;
  }
